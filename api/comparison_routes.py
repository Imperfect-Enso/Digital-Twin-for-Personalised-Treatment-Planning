...
class VitalReading(BaseModel):
    blood_pressure: float = Field(..., ge=60.0, le=250.0, example=91.4)
    glucose: float        = Field(..., ge=40.0, le=500.0, example=108.3)
    heart_rate: float     = Field(..., ge=30.0, le=220.0, example=77.2)
    inflammation: float   = Field(..., ge=0.1, le=10.0, example=1.80)
...
def list_treatments(user: str = Depends(verify_token)):
    return {
...
# Rank all treatments and save result
...
# Just return the best treatment
